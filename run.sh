...
OUTPUT_FOLDER="${OUTPUT_FOLDER:-r2r_qwen8b_6}"
# OUTPUT_FOLDER="${OUTPUT_FOLDER:-qwen3-8b_0}"
...
if [ -z "${LOGBATCHER_REQUEST_MODEL:-}" ]; then
  if [[ "${MODEL_NAME,,}" == *r2r* ]]; then
    export LOGBATCHER_REQUEST_MODEL="default"
  else
    export LOGBATCHER_REQUEST_MODEL="${MODEL_NAME}"
  fi
else
...
export LOGBATCHER_R2R_TEMPLATE_RETRY_ATTEMPTS="${LOGBATCHER_R2R_TEMPLATE_RETRY_ATTEMPTS:-3}"
export LOGBATCHER_R2R_MIN_TEMPLATE_COVERAGE="${LOGBATCHER_R2R_MIN_TEMPLATE_COVERAGE:-0.8}"
export LOGBATCHER_R2R_VALIDATE_LOGS="${LOGBATCHER_R2R_VALIDATE_LOGS:-15}"
